...
namespace Mxp.Win
{
    public sealed partial class ExpenseSelectableItemListElement : UserControl
    {
        public bool Selected { get; set; }
        private static readonly DependencyProperty InternalDataContextProperty = DependencyProperty.Register ("InternalDataContext", typeof (object), typeof (ExpenseSelectableItemListElement), new PropertyMetadata (null,DataContextChanged));

        public ExpenseSelectableItemListElement ()
        {
            this.InitializeComponent();
            this.SetBinding (InternalDataContextProperty, new Binding ());
        }
        ...
        private async void Grid_Tapped(object sender, TappedRoutedEventArgs e)
        {
            ...
            else
            {
                ...
                else if ((sender as Grid).DataContext is Expense)
                {
                    ...
                    {
                        ((Frame)Window.Current.Content).Navigate(typeof(ApprovalExpenseDetailView), expense);
                    }
                }
                else
                {
        ...
        private void SplitCheckBox_Clicked(object sender, RoutedEventArgs e)
        {
            ...
            ExpenseItem item = (sender as CheckBox).DataContext as ExpenseItem;
            item.Toggle();
        }

        private void SplitCheckBox_Loaded(object sender, RoutedEventArgs e)
        {
            ExpenseItem item = (sender as CheckBox).DataContext as ExpenseItem;
            (sender as CheckBox).IsChecked = item.StatusForApprovalReport == ExpenseItem.Status.Accepted;
        }

        private static void DataContextChanged (DependencyObject d, DependencyPropertyChangedEventArgs e) {
            ((ExpenseSelectableItemListElement)d).ItemLoaded ();
        }

        private void ItemLoaded () {
            Expense item = this.DataContext as Expense;

            if (item == null)
                return;

            if (item.IsSplit) {
                this.GridCheck.Visibility = Visibility.Collapsed;
                return;
            }

            this.SelectionCheckBox.IsChecked = item.ExpenseItems [0].StatusForApprovalReport == ExpenseItem.Status.Accepted;
        }
    }
}